# Betriebsreferenz — bei konkretem Auftrag gezielt laden

Aus dem Session-Standardkontext ausgelagert (13.09.2026). Historische Befunde
vor Verwendung live pruefen. Relative Pfade im folgenden Alttext beziehen sich
auf das urspruengliche Verzeichnis der CLAUDE.md, nicht auf diese Datei.
...
| `src/providers/gemini_vision.py` | Gemini-Bildweg per API-Key — auf der **dev**-Bridge der Standard fuer die Bildanalyse (`BRIDGE_VISION_DEFAULT_PROVIDER`) |
| `src/routing/gemini_vision_gate.py` | dessen Umgebungssperre — Staging ja, **Produktion nie** (Google ist kein gelisteter Unterauftragsverarbeiter, avv.md §5.4). Kein `GEMINI_VISION_API_KEY` auf prod, und das ist Absicht |
